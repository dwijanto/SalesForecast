...
Public Class MSImportGroup
    ...
    Public Function Run(ByVal myForm As Object, ByVal myDoc As List(Of DocCSV)) As Boolean Implements HKImport.Run
        ...
        Dim myList As New List(Of MSData)
        ...
        Dim mygroupdict As Dictionary(Of String, Integer) = GroupController1.Model.GetGroupDict(3)
            ...
            Using objTFParser = New FileIO.TextFieldParser(filename)
                With objTFParser
                    ...
                    Do Until .EndOfData
                        ...
                        ElseIf count > 9 Then
                            'data
                            myList.Add(New MSData With {.myrecord = myrecord,
        ...
        For i = 0 To myList.Count - 1
            ...
            'For j = 0 To 11
            For j = 0 To 12
                mycol = (MSReportProperty1.ColumnStartData - 1) + j
        ...
        If DATASB.Length > 0 Then
            ...
            Dim sqlstr1 = String.Format("delete from sales.sfgrouptxms tx  where kam  = '{0}' and tx.txdate >= '{1:yyyy-MM}-01' and tx.txdate < '{2}'", myDoc(0).KAM, Period, NextYear)
            PostgresqlDBAdapter1.ExecuteNonQuery(sqlstr1)

            'copy
            Dim sqlstr As String = "copy sales.sfgrouptxms(txdate,kam,groupid,cmmf,salesforecast) from stdin with null as 'Null';"
    ...
    Private Sub populatedict(mydict As Dictionary(Of Integer, Date), myrecord As String())
        'For j = 0 To 11
        For j = 0 To 12
            Dim tmpdate As Date = CDate(String.Format("{0}-{1}-1", myrecord(MSReportProperty1.ColumnStartData - 1 + j).ToString.Substring(0, 4), myrecord(MSReportProperty1.ColumnStartData - 1 + j).ToString.Substring(4, 2)))
