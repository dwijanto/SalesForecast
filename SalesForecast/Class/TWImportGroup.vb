...
Public Class TWImportGroup
    ...
    Public Function Run(ByVal myForm As Object, ByVal myDoc As List(Of DocCSV)) As Boolean Implements HKImport.Run
        ...
        Dim myList As New List(Of TWData)
            ...
            Using objTFParser = New FileIO.TextFieldParser(filename)
                With objTFParser
                    ...
                    Do Until .EndOfData
                        ...
                        ElseIf count > 8 Then
                            'data
                            myList.Add(New TWData With {.myrecord = myrecord,
                                                        .Group = myDoc(i).FG,
                                                        .KAM = myDoc(i).KAM
                                                        })
                        End If
        ...
        For i = 0 To myList.Count - 1
            ...
            'For j = 0 To 11
            For j = 0 To 12
                mycol = (TWReportProperty1.ColumnStartData - 1) + j
                If myList(i).myrecord(mycol) <> "" Then

                    DATASB.Append(String.Format("'{0:yyyy-MM-dd}'", myPeriodDict(j)) & vbTab &
                                  myList(i).KAM & vbTab &
                                  myList(i).Group & vbTab & myList(i).myrecord(1) & vbTab & myList(i).myrecord(mycol) & vbCrLf)
                End If
            Next
        Next
        ...
        If DATASB.Length > 0 Then
            ...
            Dim sqlstr1 = String.Format("delete from sales.sfgrouptxtw tx  where kam  = '{0}' and tx.txdate >= '{1:yyyy-MM}-01' and tx.txdate < '{2}'", myDoc(0).KAM, Period, NextYear)
            PostgresqlDBAdapter1.ExecuteNonQuery(sqlstr1)

            'copy
            Dim sqlstr As String = "copy sales.sfgrouptxtw(txdate,kam,groupname,cmmf,salesforecast) from stdin with null as 'Null';"
    ...
    Private Sub populatedict(mydict As Dictionary(Of Integer, Date), myrecord As String())
        'For j = 0 To 11
        For j = 0 To 12
            Dim tmpdate As Date = CDate(String.Format("{0}-{1}-1", myrecord(TWReportProperty1.ColumnStartData - 1 + j).ToString.Substring(0, 4), myrecord(TWReportProperty1.ColumnStartData - 1 + j).ToString.Substring(4, 2)))
