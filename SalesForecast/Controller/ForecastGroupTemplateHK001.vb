...
Public Class ForecastGroupTemplateHK001
        ...
        If (mysaveform.ShowDialog() = Windows.Forms.DialogResult.OK) Then
            ...
            If e.blankTemplate Then
                ...
                sqlstr = String.Format("with tx as (select * from crosstab('with tx as(select cmmf,txdate,sum(salesforecast) as salesforecast from sales.sfmlatxhk tx " &
                                        ...
                                        " select * from alltx order by 1,2','select unnest(Array[{1}])::date') " &
                                        " as ct (cmmf bigint, {2})) ,nsp as (select cmmf,nsp1, nsp2 from sales.sfcmmfnsp )" &
                                        " select c.cmmf,c.reference,c.description ,c.brand,c.familyname,sales.get_producttype(productlinegps,brand) as producttype,n.nsp1,n.nsp2 ,{3} from sales.sfcmmf c " &
                                        " left join tx on tx.cmmf = c.cmmf left join nsp n on n.cmmf = c.cmmf ", myPeriodRange(0), fieldList.ToString, ColumnList.ToString, TxtFieldList.ToString)
            Else
                'sqlstr = String.Format("with tx as (select * from crosstab('with tx as(select cmmf,txdate,sum(salesforecast) as salesforecast from sales.sfmlatxhk tx " &
                '                       " where txdate >= ''{0:yyyy-MM-dd}''" &
                ...
                If Identitiy.isAdmin Then
                    ...
                    '                                  " select * from alltx order by 1,2','select unnest(Array[{2}])::date') " &
                    '                                  " as ct (cmmf bigint, {3})),nsp as (select cmmf,nsp1,nsp2 from sales.sfcmmfnsp )" &
                    '                                  " select c.cmmf,c.reference,c.description,c.brand,c.familyname,sales.get_producttype(productlinegps,brand) as producttype,n.nsp1,n.nsp2 ,{4} from sales.sfcmmf c " &
                    '                                  " left join tx on tx.cmmf = c.cmmf left join nsp n on n.cmmf = c.cmmf ", myPeriodRange(0), myPeriodRange(6), fieldList.ToString, ColumnList.ToString, TxtFieldList.ToString)
                    'sqlstr = String.Format("with tx as (select * from crosstab('with tx as(select cmmf,txdate,sum(salesforecast) as salesforecast from sales.sfmlatxhk tx " &
                    '                   " where txdate >= ''{0:yyyy-MM-dd}''" &
                    '                                  " group by cmmf,txdate),mla as(select c.cmmf,tx.txdate,tx.salesforecast from sales.sfcmmf c left join tx on tx.cmmf = c.cmmf " &
                    ...
                    '                                  " select * from alltx order by 1,2','select unnest(Array[{2}])::date') " &
                    '                                  " as ct (cmmf bigint, {3})),nsp as (select cmmf,nsp1,nsp2 from sales.sfcmmfnsp )" &
                    '                                  " select c.cmmf,c.reference,c.description,c.brand,c.familyname,sales.get_producttype(c.productlinegpsid,brand) as producttype,n.nsp1,n.nsp2 ,{4} from sales.sfcmmf c " &
                    '                                  " left join tx on tx.cmmf = c.cmmf left join nsp n on n.cmmf = c.cmmf ", myPeriodRange(0), myPeriodRange(6), fieldList.ToString, ColumnList.ToString, TxtFieldList.ToString)
                    sqlstr = String.Format("with tx as (select * from crosstab('with tx as(select cmmf,txdate,sum(salesforecast) as salesforecast from sales.sfmlatxhk tx " &
                                       " where txdate >= ''{0:yyyy-MM-dd}''" &
                                                      " group by cmmf,txdate),mla as(select c.cmmf,tx.txdate,tx.salesforecast from sales.sfcmmf c left join tx on tx.cmmf = c.cmmf " &
                                                      ...
                                                      " select * from alltx order by 1,2','select unnest(Array[{2}])::date') " &
                                                      " as ct (cmmf bigint, {3})),nsp as (select cmmf,nsp1,nsp2 from sales.sfcmmfnsp )" &
                                                      " select c.cmmf,c.reference,c.description,c.brand,to_char(c.familyid,'FM000') || ' - ' || f.familyname || ' - ' || f.familylv2 as familyname,sales.get_producttype(c.productlinegpsid,brand) as producttype,n.nsp1,n.nsp2 ,{4} from sales.sfcmmf c " &
                                                      " left join tx on tx.cmmf = c.cmmf left join nsp n on n.cmmf = c.cmmf left join sales.sffamily f on f.familyid = c.familyid", myPeriodRange(0), myPeriodRange(6), fieldList.ToString, ColumnList.ToString, TxtFieldList.ToString)
                ...
                Else
                    ...
                    sqlstr = String.Format("with tx as (select * from crosstab('with tx as(select cmmf,txdate,sum(salesforecast) as salesforecast from sales.sfmlatxhk tx " &
                                      " where txdate >= ''{0:yyyy-MM-dd}''" &
                                                     ...
                                                     " as ct (cmmf bigint, {3})),nsp as (select cmmf,nsp1,nsp2 from sales.sfcmmfnsp )" &
                                                     " select c.cmmf,c.reference,c.description,c.brand,to_char(c.familyid,'FM000') || ' - ' || f.familyname || ' - ' || f.familylv2 as familyname,sales.get_producttype(c.productlinegpsid,brand) as producttype,n.nsp1,n.nsp2 ,{4} from sales.sfcmmf c " &
                                                     " left join tx on tx.cmmf = c.cmmf left join nsp n on n.cmmf = c.cmmf  left join sales.sffamily f on f.familyid = c.familyid", myPeriodRange(0), myPeriodRange(6), fieldList.ToString, ColumnList.ToString, TxtFieldList.ToString, Identitiy.getId)
