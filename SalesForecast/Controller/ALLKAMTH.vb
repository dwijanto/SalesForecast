...
Public Class ALLKAMTH
    ...
    Public Sub Generate(myForm As Object, e As ALLKAMEventArgs)
        ...
        mysaveform.FileName = String.Format("SalesForecastTHALL{0:yyyyMMdd}.xlsx", Date.Today)
        ...
        If (mysaveform.ShowDialog() = Windows.Forms.DialogResult.OK) Then
            ...
            sqlstr = String.Format("with cc as (select cmmf, case when productline like '%COOKWARE%' then 'Cookware' else 'SDA' end as producttype from sales.sfcmmfth)" &
                                   " select  ka.kam,g.groupname,mg.mla,mc.account,cc.producttype,c.cmmf,c.productline,c.familylv2,c.commercialcode,c.itemdescription,c.status,c.rsp,c.cogs," &
                                   " case when productline like '%COOKWARE' then  c.rsp * (1 - gpcw) / 1.07 * (1-ifrscw) else c.rsp * (1 - gpsda) / 1.07 * (1-ifrssda) end as nnsp," &
                                   " gpcw, gpsda, ifrscw, ifrssda, tx.txdate, tx.salesforecast as qty,case when productline like '%COOKWARE' then  c.rsp * (1 - gpcw) / 1.07 * (1-ifrscw) * salesforecast else c.rsp * (1 - gpsda) / 1.07 * (1-ifrssda) * salesforecast end as amount" &
                                   " from sales.sfgrouptxth tx left join sales.sfcmmfth c on c.cmmf = tx.cmmf left join sales.sfkamassignmentth ka on ka.id = tx.kamassignmentid" &
                                   " left join sales.sfkam k on k.username = ka.kam left join sales.sfmlacardnameth mc on mc.id = ka.mlacardnameid left join sales.sfmlagroupth mg on mg.mla = mc.mla" &
                                   " left join cc on cc.cmmf = tx.cmmf left join sales.sfgroup g on g.id = mg.groupid   where tx.txdate >= '{0:yyyy-MM-}01' and tx.txdate<= '{1:yyyy-MM-}01' " &
                                   " and not c.cmmf isnull and k.isactive order by kam,mla,groupname,producttype,cmmf,productline,familylv2,commercialcode,txdate", e.startperiod, e.endperiod)
    ...
    Private Sub PivotTable(ByRef sender As Object, ByRef e As EventArgs)
        ...
        osheet.PivotTables("PivotTable1").Pivotfields("familylv2").orientation = Excel.XlPivotFieldOrientation.xlRowField
        osheet.PivotTables("PivotTable1").Pivotfields("cmmf").orientation = Excel.XlPivotFieldOrientation.xlRowField
        osheet.PivotTables("PivotTable1").Pivotfields("itemdescription").orientation = Excel.XlPivotFieldOrientation.xlRowField
