...
Public Class APOMY
    Public Sub Generate(myForm As Object, e As APOMYEventArgs)

        Dim sqlstr As String = String.Empty '= "select * from sales.sfcmmf;"

        Dim mysaveform As New SaveFileDialog
        mysaveform.FileName = String.Format("3730 GSMY_{0:yyyyMMdd}.xlsx", e.startPeriod)
        ...
        If (mysaveform.ShowDialog() = Windows.Forms.DialogResult.OK) Then
            ...
            '         " left join sales.sfgroupmlams gm on gm.groupid =  tx.groupid where  tx.txdate >= '{0:yyyy-MM}-01' and tx.txdate < '{1}'" &
            '         " group by ""Calendar year/Month"",""CMMF"",""Market"",""Log.Distrib.Centre"" ,mla", e.startPeriod, NextYear)
            ...
            sqlstr = String.Format("select 'NR', to_char(txdate,'YYYYMM') as ""Calendar year/Month"",tx.cmmf as ""CMMF"", 6710 as  ""Log.Distrib.Centre"" ,mla as ""Main Local Account"",null::text as ""Forecast Group"",null::text as ""Sales Force"", null::text as ""Sales Organization""," &
                     " null::text as ""Sales Group"",sum(salesforecast) as ""Commercial Qty"",'PC' from sales.sfgrouptxms tx" &
                     " left join sales.sfgroupmlams gm on gm.groupid =  tx.groupid where  tx.txdate >= '{0:yyyy-MM}-01' and tx.txdate < '{1}'" &
                     " group by ""Calendar year/Month"",""CMMF"",""Log.Distrib.Centre"" ,mla", e.startPeriod, NextYear)
            Dim myreport As New ExportToExcelFile(myForm, sqlstr, filename, reportname, mycallback, PivotCallback, datasheet, "\templates\ExcelTemplateMS.xltx", "A2", False)
